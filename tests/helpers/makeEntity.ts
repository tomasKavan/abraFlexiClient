...
export type MakeEntityOptions = {
  /**
   * Map of supported business actions (mirrors the per-entity `static Actions`
   * that the generator emits, e.g. `{ Storno: 'storno' }`). When omitted,
   * the test class has no actions and AFApiClient.action() will reject any
   * actionName as unsupported.
   */
 ...
 */
export function makeEntityClass(
  props: PropMap = { name: PropertyType.String },
  options: MakeEntityOptions = {}
) {
  ...
  class TestEntity extends AFEntity {
    ...
    static override Actions: Record<string, string> = options.actions ?? {}
  }
